...
def test_save_and_load_cache(provider):
    df = pd.DataFrame({
        CandleCol.DATETIME: pd.date_range("2023-02-01", periods=5, freq="2min", tz="UTC"),
...
def test_fetch_and_cache_new_data(mock_client, provider):
    df = pd.DataFrame({
        CandleCol.DATETIME: pd.date_range("2023-03-01", periods=3, freq="1min", tz="UTC"),
...
@patch("fastscanner.adapters.candle.partitioned_csv.PartitionedCSVBarsProvider._cache")
...
def test_get_trims_data_within_start_end(mock_cache):
    ...
    with patch.object(provider, "_partition_keys_in_range", return_value=["2023-01"]):
        ...
        assert all((result.index.date >= date(2023, 1, 2)) & (result.index.date <= date(2023, 1, 3)))
...
def test_cache_fallback_on_corrupt_file(mock_fetch, provider):
    ...
    df = pd.DataFrame({
        CandleCol.DATETIME: pd.date_range("2023-04-01", periods=2, freq="1min", tz="UTC"),
