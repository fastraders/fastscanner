...
class CumulativeDailyVolumeIndicator:
    ...
    async def load_from_cache(self, symbol: str | None = None):
        ...
        self._last_volume = {
            k: v
            for k, v in indicator_data["last_volume"].items()
            if symbol is None or k == symbol
        }
    ...
    async def extend_realtime(self, symbol: str, new_row: Candle) -> Candle:
        ...
        if last_date is not None and last_date == new_row.timestamp.date():
...
class PremarketCumulativeIndicator:
    ...
    async def extend_realtime(self, symbol: str, new_row: Candle) -> Candle:
        ...
        if last_value is not None and last_date == new_row.timestamp.date():
...
class ATRIndicator:
    ...
    async def load_from_cache(self, symbol: str | None = None):
        ...
        self._last_atr = {
            k: v
            for k, v in indicator_data["last_atr"].items()
            if symbol is None or k == symbol
        }
        self._last_close = {
            k: v
            for k, v in indicator_data["last_close"].items()
            if symbol is None or k == symbol
        }
    ...
    async def extend_realtime(self, symbol: str, new_row: Candle) -> Candle:
        ...
        if last_close is None:
            ...
            self._last_atr[symbol] = float(result[self.column_name()])
            self._last_close[symbol] = float(result[CandleCol.CLOSE])
            new_row.update(result.to_dict())
            return new_row
...
class PositionInRangeIndicator:
    ...
    async def extend_realtime(self, symbol: str, new_row: Candle) -> Candle:
        original_keys = set(new_row.keys())
        ...
        columns_to_drop = [
            col for col in [high_col, low_col] if col not in original_keys
        ]
        new_row.drop(columns_to_drop)
...
class DailyRollingIndicator:
    ...
    async def load_from_cache(self, symbol: str | None = None):
        ...
        self._rolling_values = {
            k: v
            for k, v in indicator_data["rolling_values"].items()
            if symbol is None or k == symbol
        }
        self._last_date = {
            k: date.fromisoformat(v)
    ...
    async def extend_realtime(self, symbol: str, new_row: Candle) -> Candle:
        ...
        if last_date is None or last_date != new_row.timestamp.date():
            daily_df = await self._get_data_for_n_days(symbol, new_row.to_dataframe())
            self._last_date[symbol] = new_row.timestamp.date()
...
class GapIndicator:
    ...
    async def load_from_cache(self, symbol: str | None = None):
        await self._prev_day.load_from_cache(symbol)
...
class ATRGapIndicator:
    ...
    async def load_from_cache(self, symbol: str | None = None):
        await self._atr.load_from_cache(symbol)
        await self._gap.load_from_cache(symbol)
        await self._prev_day.load_from_cache(symbol)
...
class ShiftIndicator:
    ...
    async def load_from_cache(self, symbol: str | None = None):
        ...
        self._last_values = {
            k: v
            for k, v in indicator_data["last_values"].items()
            if symbol is None or k == symbol
        }
