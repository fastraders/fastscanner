...
@pytest.mark.asyncio
async def test_subscribe_realtime_creates_subscriptions_for_all_symbols(
...
@pytest.mark.asyncio
async def test_subscribe_realtime_uses_scanner_library(
    scanner_service, subscription_handler
):
    params = ScannerParams(type_="another_dummy", params={"threshold": 1000})
...
@pytest.mark.asyncio
async def test_subscribe_realtime_returns_unique_scanner_ids(
    scanner_service, scanner_params, subscription_handler
):
...
@pytest.mark.asyncio
async def test_unsubscribe_realtime_removes_all_subscriptions(
    scanner_service, scanner_params, subscription_handler
):
    scanner_id = await scanner_service.subscribe_realtime(
        params=scanner_params, handler=subscription_handler, freq="1min"
...
@pytest.mark.asyncio
async def test_subscribe_realtime_uses_symbols_provider(
    scanner_service, scanner_params, subscription_handler
):
...
@pytest.mark.asyncio
async def test_scanner_params_with_different_frequencies(
    scanner_service, scanner_params, subscription_handler
):
    scanner_id = await scanner_service.subscribe_realtime(
        params=scanner_params, handler=subscription_handler, freq="5min"
    )
...
@pytest.mark.asyncio
async def test_scanner_concurrent_subscription_execution(
    scanner_service, scanner_params, subscription_handler
):
...
@pytest.mark.asyncio
async def test_scanner_service_multiple_concurrent_subscriptions(
    scanner_service, subscription_handler
):
    ...
    scanner_id_1 = await scanner_service.subscribe_realtime(
        params1, subscription_handler, "1min"
    )
    scanner_id_2 = await scanner_service.subscribe_realtime(
        params2, subscription_handler, "1min"
    )
...
@pytest.mark.asyncio
async def test_scanner_service_limits_symbol_count(
    scanner_service, scanner_params, subscription_handler
):
